...
## Checkpoint 21L-A — Pilot Sales Readiness: Public Marketing Pages

**Status: Complete ✓ — pnpm tsc --noEmit and pnpm build pass**

### What was added

A public-facing marketing/sales layer for Court Time built as a `(marketing)` route
group. All pages are public (no authentication required). The existing authenticated
`(app)` routes are completely unchanged.

### Routes

| Route | Handler | Auth | Build type |
|---|---|---|---|
| `/` | `(marketing)/page.tsx` | Redirects logged-in users to `/calendar` | `ƒ` Dynamic |
| `/pricing` | `(marketing)/pricing/page.tsx` | None | `○` Static |
| `/contact` | `(marketing)/contact/page.tsx` | None | `○` Static |
| `/terms` | `(marketing)/terms/page.tsx` | None | `○` Static |
| `/privacy` | `(marketing)/privacy/page.tsx` | None | `○` Static |

### Root route change

`src/app/page.tsx` (previously: redirect to `/calendar` or `/sign-in`) has been
deleted. It is replaced by `src/app/(marketing)/page.tsx` which:
- Redirects authenticated users → `/calendar` (same as before)
- Shows the landing page for unauthenticated users (new behavior)
...
src/app/
  (marketing)/
    layout.tsx          — Marketing shell: nav + footer, no app chrome
    ...
    components/
      MarketingNav.tsx  — Sticky header: wordmark, Pricing, Contact, Sign in
      MarketingFooter.tsx — Footer: Terms, Privacy, Contact, Sign in + copyright
```
...
| File | Change |
|---|---|
| `src/app/(marketing)/layout.tsx` | **New** — marketing shell layout |
...
- Trial or self-serve signup flow
- Contact form backend (currently mailto only)
- Calendly/Cal.com scheduling embed
- Analytics (GA4, Plausible, etc.)
- SEO: sitemap, og:image, structured data
- Blog / changelog
- Testimonials with real copy
...
- [ ] `/contact` loads without auth
- [ ] `/terms` loads without auth
- [ ] `/privacy` loads without auth

**Marketing layout:**
- [ ] Nav header visible on all marketing pages
- [ ] Nav includes: Court Time wordmark, Pricing, Contact, Sign in
- [ ] On mobile: Pricing link hidden, Contact visible, Sign in visible
- [ ] Footer visible on all marketing pages with Terms, Privacy, Contact, Sign in
- [ ] No SideNav or BottomNav on marketing pages
- [ ] Dark mode renders correctly on all marketing pages

**Landing page (`/`):**
- [ ] Hero headline visible: "Less chaos. More tennis."
- [ ] "Request early access" button links to /contact
- [ ] "Sign in →" button links to /sign-in
- [ ] Pain/problem section visible
- [ ] 4 feature cards render correctly
- [ ] How it works (3 steps) renders correctly
...
- [ ] Email mailto: hello@court-time.app is clickable
- [ ] Scheduling placeholder copy visible
- [ ] Link to /pricing visible

**Legal pages:**
- [ ] /terms loads and shows Last updated: July 7, 2026
- [ ] /privacy loads and shows Last updated: July 7, 2026
- [ ] Both pages have readable content (not lorem ipsum)
- [ ] Both pages have email contact link

**Authenticated app (must not regress):**
- [ ] /calendar still works for logged-in users
- [ ] /admin/members still works
- [ ] /events still works
- [ ] /my-schedule still works
- [ ] App layout (SideNav + BottomNav) only appears on /app/* routes
- [ ] pnpm tsc --noEmit ✓ / pnpm build ✓

---

## Checkpoint 21L-A addendum — Marketing Motion Polish + Pricing Adjustment

**Status: Complete ✓ — pnpm tsc --noEmit and pnpm build pass**

### Motion/interaction polish added

**Page entrance animation (`template.tsx`):**
`(marketing)/template.tsx` wraps each marketing page in `<div class="mkt-page-enter">`.
CSS: `mkt-page-enter` = fade-in + `translateY(8px → 0)` over 320ms ease-out.
Re-plays on every navigation within the marketing group since `template.tsx`
re-mounts on route changes (unlike `layout.tsx`).

**Scroll-reveal component (`MarketingReveal.tsx`):**
Client component using `IntersectionObserver`. Elements start with `opacity:0 +
translateY(20px)`, gain `.mkt-reveal--visible` when 10% visible in viewport.
Observer fires once (unobserves after reveal). Threshold has a 40px bottom rootMargin
so elements reveal slightly before the bottom edge is reached.
Supports stagger delays: `delay-1` (80ms), `delay-2` (160ms), `delay-3` (240ms).

**Reduced motion:** Under `prefers-reduced-motion: reduce`, the `.mkt-reveal` class
overrides to `opacity:1 + transform:none + transition:none` so content is always
immediately visible.

**Applied to:**
- Landing page: pain section, features heading, all 4 feature cards (staggered),
  "How it works" heading, all 3 steps (staggered)
- Pricing page: header, founding club card, future plans card, each FAQ item
- Contact page: header + intro, "helpful to mention" card, email CTA card, call card

**Hover upgrades:**
- Feature cards: `hover:shadow-md` (upgraded from `hover:shadow-sm`)
- Founding Club pricing card: `hover:shadow-xl`
- Future plan sub-cards: `hover:shadow-sm` + border lightens
- Contact primary card: `hover:shadow-lg` + `border-2` to match pricing card weight
- CTA buttons: added `active:scale-[0.98]` for pressed feedback

### Pricing copy changes

| Plan | Before | After |
|---|---|---|
| Starter monthly | $199 / month | $249 / month |
| Starter annual | $1,990 / yr | $2,490 / yr |
| Club monthly | $349 / month | $399 / month |
| Club annual | $3,490 / yr | $3,990 / yr |
| Founding Club | Unchanged | $149/mo or $1,490/yr |
...
| `src/app/(marketing)/components/MarketingReveal.tsx` | **New** — scroll-reveal component |
| `src/app/globals.css` | Added `mkt-page-enter` + `mkt-reveal` animation/reveal CSS |
| `src/app/(marketing)/page.tsx` | Added MarketingReveal wrappers to all sections |
| `src/app/(marketing)/pricing/page.tsx` | Updated Starter/Club prices; added reveal wrappers + hover |
| `src/app/(marketing)/contact/page.tsx` | Added reveal wrappers + hover |

### QA checklist addendum

**Page entrance animation:**
- [ ] Navigating to / shows a soft fade-in on the page content
- [ ] Navigating between /pricing and /contact re-plays the entrance animation
- [ ] Under prefers-reduced-motion: no entrance animation fires

**Scroll-reveal:**
- [ ] Landing page feature cards fade in as you scroll down
- [ ] Pricing cards reveal as they enter viewport
- [ ] Contact cards reveal as they enter viewport
- [ ] Under prefers-reduced-motion: all elements visible immediately (no fade)
- [ ] Elements already in view on page load appear immediately without delay

**Hover effects:**
- [ ] Feature cards show stronger shadow on hover
- [ ] Founding Club pricing card shows elevated shadow on hover
- [ ] Contact email CTA card shows shadow on hover
- [ ] CTA buttons show pressed scale on click

**Pricing numbers (updated):**
- [ ] Pricing page shows $149/month or $1,490/year for Founding Club
- [ ] Pricing page shows $249/month or $2,490/year for Starter
- [ ] Pricing page shows $399/month or $3,990/year for Club
- [ ] Annual plans note "two months free"
- [ ] No "locked in for life" language present
