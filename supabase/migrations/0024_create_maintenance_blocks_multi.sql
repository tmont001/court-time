-- 0024_create_maintenance_blocks_multi.sql
-- Phase 10E: multi-court maintenance block creation.
-- Adds create_maintenance_blocks (plural) RPC that accepts an array of court IDs
-- and inserts one maintenance reservation per court in a single transaction.
-- The existing create_maintenance_block (singular) is preserved for compatibility.
...
begin
  ...
  -- Validate every submitted court belongs to the caller's club and is active.
  if exists (
  ...
  -- Insert one reservation per court. The EXCLUDE constraint on reservations
  -- prevents overlapping pending/confirmed rows; any conflict raises 23P01 and
  -- rolls back the whole transaction so no partial state is persisted.
