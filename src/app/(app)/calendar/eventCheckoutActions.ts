  ...
  if (resolveError || !resolvedRows || resolvedRows.length === 0) {
    const key =
      resolveError?.message.match(
        /capability_not_available|event_not_found|event_not_scheduled|event_participant_not_found|event_participant_not_confirmed|not_online_payable|payment_not_open_for_checkout|no_balance_due|payment_not_found|stale_attempt_environment_mismatch|invalid_arguments/,
      )?.[0] ?? "";
    ...
    if (supersedeError || !supersededRows || supersededRows.length === 0) {
      const key =
        supersedeError?.message.match(
          /capability_not_available|event_not_found|event_not_scheduled|event_participant_not_found|event_participant_not_confirmed|not_online_payable|payment_not_open_for_checkout|no_balance_due|payment_not_found|checkout_attempt_not_found|invalid_arguments/,
        )?.[0] ?? "";
      if (!ERROR_MESSAGES[key]) {
